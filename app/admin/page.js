      ...
      <aside className="w-64 bg-white/80 backdrop-blur-md shadow-md p-6">
        ...
        <nav className="flex flex-col gap-3">
          <button
            ...
            className={`text-left px-4 py-2 rounded-lg transition ${activeTab === "products"
          ...
          <button
            ...
            className={`text-left px-4 py-2 rounded-lg transition ${activeTab === "categories"
              ? "bg-blue-600 text-white"
              : "hover:bg-blue-100"
              }`}
          >
          ...
          <button
            ...
            className={`text-left px-4 py-2 rounded-lg transition ${activeTab === "orders"
              ? "bg-blue-600 text-white"
              : "hover:bg-blue-100"
              }`}
          >
          ...
          <button
            ...
            className={`text-left px-4 py-2 rounded-lg transition ${activeTab === "settings"
              ? "bg-blue-600 text-white"
              : "hover:bg-blue-100"
              }`}
          >
...
function ProductsAdmin() {
  ...
  const availableSubcategories = categories.find((cat) => cat.name === newProduct.category)?.subcategories || [];
  ...
  return (
    <div className="space-y-10">
        ...
        <div className="flex gap-2">
          {[
            ...
            <button
              ...
              className={`px-3 py-1 rounded-lg border ${mode === item.key
      ...
      <form onSubmit={handleAddProduct} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 bg-white shadow p-6 rounded-xl">
        ...
        <select value={newProduct.category} onChange={e => setNewProduct({ ...newProduct, category: e.target.value, subcategory: "" })} className="border p-2 rounded" required>
          <option value="">Select Category</option>
          {categories.map(cat => <option key={cat._id} value={cat.name}>{cat.name}</option>)}
        </select>
        {availableSubcategories.length > 0 && (
          <select value={newProduct.subcategory} onChange={e => setNewProduct({ ...newProduct, subcategory: e.target.value })} className="border p-2 rounded" required>
        ...
        <input
          ...
          value={newProduct.voteCount ?? 0}
          onChange={e =>
            setNewProduct({
              ...newProduct,
              voteCount: Number(e.target.value),
            })
          }
        ...
        <input
          ...
          value={newProduct.productCounter ?? 0}
          onChange={e =>
            setNewProduct({
              ...newProduct,
              productCounter: Number(e.target.value),
            })
          }
          className="border p-2 rounded"
        />
...
function CategoriesAdmin() {
  ...
  async function handleAddSubcategory(e, category) {
    e.preventDefault();
    const value = (subcategoryInputs[category._id] || "").trim();
    if (!value) return;

    const updatedSubcategories = [...(category.subcategories || []), value.toLowerCase()];
    const res = await fetch("/api/categories", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: category._id, subcategories: updatedSubcategories }),
    });
  ...
  async function handleRemoveSubcategory(category, subcategory) {
    if (!confirm(`Remove subcategory "${subcategory}" from ${category.name}?`)) return;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {categories.map((cat) => (
          <div key={cat._id} className="bg-white p-4 rounded shadow space-y-3">
                ...
                {cat.subcategories?.length > 0 ? (
                  <div className="text-sm text-gray-500 mt-2 space-y-2">
                    {cat.subcategories.map((sub) => (
                      <div key={sub} className="flex items-center justify-between gap-2 rounded border border-gray-200 bg-gray-50 px-3 py-2">
                  ...
                  <p className="text-sm text-gray-400 mt-1">No subcategories yet</p>
                )}
              </div>
              <button onClick={() => handleDeleteCategory(cat._id)} className="text-red-600 hover:text-red-800">Delete</button>
            </div>
            <form onSubmit={(e) => handleAddSubcategory(e, cat)} className="flex gap-2">
              <input
                value={subcategoryInputs[cat._id] || ""}
                onChange={(e) => setSubcategoryInputs((prev) => ({ ...prev, [cat._id]: e.target.value }))}
                placeholder="Add subcategory"
                className="border p-2 rounded flex-1"
              />
              <button className="bg-blue-600 px-3 py-2 text-white rounded">Add</button>
            </form>
          </div>
        ))}
      </div>
