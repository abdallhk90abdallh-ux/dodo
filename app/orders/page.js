...
export default function OrdersPage() {
  ...
  const orderCards = orders.map((order) => {
    const sortedItems = (order.items || []).slice().sort((a, b) => {
      const aSize = a.size ? Number(a.size) : Number.NaN;
      const bSize = b.size ? Number(b.size) : Number.NaN;

      if (!Number.isNaN(aSize) && !Number.isNaN(bSize)) return aSize - bSize;
    ...
    return (
      <div key={order._id} className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100 hover:shadow-md transition">
                  ...
                  <div className="mt-2 flex items-center gap-1">
                    {[1, 2, 3, 4, 5].map((s) => (
                      <button
                        key={s}
                        disabled={isRated}
                        onMouseEnter={() => !isRated && setHoveredStars((prev) => ({ ...prev, [pid]: s }))}
                        onMouseLeave={() => !isRated && setHoveredStars((prev) => ({ ...prev, [pid]: 0 }))}
                        onClick={async () => {
                          if (isRated) return;
                          try {
                            ...
                            setRatedProducts((prev) => ({ ...prev, [pid]: s }));
                            setHoveredStars((prev) => ({ ...prev, [pid]: 0 }));
                        ...
                        className={`text-xl leading-none transition ${s <= previewValue ? getStarColor(previewValue) : "text-gray-300"}`}
        ...
        <div className="mt-4 flex justify-between text-sm text-gray-600">
          ...
          <p className="font-semibold text-gray-900">Total: EGP {order.total}</p>
        </div>
      </div>
    );
  });
